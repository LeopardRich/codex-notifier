...
fn diagnose_windows_permissions(path: &Path) -> DiagnosticStatus {
    const SCRIPT: &str = r"$ErrorActionPreference='Stop';$p=$env:CODEX_NOTIFIER_AUTHORIZED_KEYS;$acl=Get-Acl -LiteralPath $p;$me=[Security.Principal.WindowsIdentity]::GetCurrent().User.Value;$allowed=@($me,'S-1-5-18','S-1-5-32-544');$owner=$acl.Owner;try{$owner=(New-Object Security.Principal.NTAccount($owner)).Translate([Security.Principal.SecurityIdentifier]).Value}catch{};$unsafe=$false;foreach($rule in $acl.Access){try{$sid=$rule.IdentityReference.Translate([Security.Principal.SecurityIdentifier]).Value}catch{$unsafe=$true;continue};$write=($rule.FileSystemRights -band [Security.AccessControl.FileSystemRights]::WriteData) -or ($rule.FileSystemRights -band [Security.AccessControl.FileSystemRights]::Modify) -or ($rule.FileSystemRights -band [Security.AccessControl.FileSystemRights]::FullControl);if($rule.AccessControlType -eq 'Allow' -and $write -and $allowed -notcontains $sid){$unsafe=$true}};if((-not $acl.AreAccessRulesProtected)-or($allowed -notcontains $owner)-or$unsafe){exit 3};exit 0";
