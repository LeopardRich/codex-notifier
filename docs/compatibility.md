...
| --- | --- | --- | --- | --- |
| Windows 10 22H2+ x86-64 desktop | Local native notifications and remote receiver | Local source-to-WinRT and continuous restricted OpenSSH-to-WinRT paths verified with current optimized builds | `codex-notifier-v0.1.0-windows-x86_64.zip` passed unsigned verification lifecycle | Blocked on production Authenticode identity and protected candidate rerun |
| macOS 14+ Intel/Apple-silicon desktop | Local native notifications and remote receiver | Local native evidence on macOS 14/current macOS and continuous restricted OpenSSH-to-UserNotifications evidence on macOS 14 | `codex-notifier-v0.1.0-macos-universal.zip` passed ad-hoc verification lifecycle | Blocked on Developer ID, notarization, and protected candidate rerun |
...
Linux has no native notification adapter. Each remote desktop row is supported
by its own continuous platform run; those optimized source-build runs are not
signed-candidate evidence. Windows Server is unsupported, the ordinary CLI
approval hook remains unverified, and version `0.1.0` has no previous stable
package for a literal upgrade. These limits must remain visible in release
