...
> 当前状态：阶段 01-08 已完成，兼容性证据、架构决策、Rust workspace、三平台
> 质量门禁、规范事件领域模型、分层配置、跨平台路径规则与结构化脱敏日志模型均已
> 建立，事务性 SQLite 发件箱/去重存储与有界的用户级本地 IPC 也已完成；SSH 与
> 原生通知适配器尚未实现。阶段 09 的 agent 生命周期实现正等待三平台验证。
...
| 产品事件 | 所需 Codex 能力 | 规划行为 |
| --- | --- | --- |
| 任务完成 | 可供外部程序调用的轮次完成通知或 hook 事件 | 规范化载荷并入队为 `task_completed`。 |
...
读取私有会话日志作为静默降级方案。正式编码前，应以目标 Codex CLI 版本核对
适配器契约；本项目目前不承诺所有 Codex 版本都能向外部程序暴露上述两类事件。
...
最终命令名称可能在实现阶段调整，但职责必须保持分离：

| 命令 | 用途 |
| --- | --- |
| `agent` | 运行桌面端或中继端用户级进程。 |
| `emit` | 面向 Codex 的快速本地事件入口。 |
| `receive` | 桌面端面向受限 SSH 的事件入口。 |
| `install` / `uninstall` | 管理 Codex 集成与用户自启动产物。 |
| `doctor` | 执行只读能力与连通性检查。 |
| `test` | 显式发送模拟通知或端到端测试事件。 |
| `status` | 显示 agent、队列与最近投递状态，不展示事件正文。 |
